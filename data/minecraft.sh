...
mc_saveon() {
	...
	then
		...
		as_user "screen -p 0 -S minecraft -X eval 'stuff \"say Sauvegarde termin̩e!\"\015'"
	else
...
mc_stop() {
	...
	then
		...
		as_user "screen -p 0 -S minecraft -X eval 'stuff \"say Arr̻t du serveur dans 10 secondes!\"\015'"
		as_user "screen -p 0 -S minecraft -X eval 'stuff \"save-all\"\015'"
		sleep 10
...
mc_update() {
	...
	then
		...
		MC_SERVER_URL=`wget -q -O - http://www.minecraft.net/download | grep minecraft_server | grep exe | cut -d \" -f 2 | sed s/exe/jar/g`
